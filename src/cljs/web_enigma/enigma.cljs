...
(s/def ::valid-alpha-string? (s/and string? 
                                    #(not (empty? %))
                                    #(re-seq #"^[A-Za-z]*$" %)))
...
returns back the entire enigma machine"
  [{:keys [reflector left-rotor middle-rotor right-rotor letter plugboard settings] :as e-machine}]
...
(defn enigma-machine
  ...
  (when-not (s/valid? ::enigma-machine? e-machine)
    (throw (ex-info (str "invalid input " "invalid enigma machine configuration"
                         #_(s/explain-str ::enigma-machine? e-machine))
                    {})) )
  (when-not (s/valid? ::unique-rotors [(:wiring left-rotor)
                                           (:wiring middle-rotor) 
                                           (:wiring right-rotor)])
    (throw (ex-info (str "invalid input: need unique rotors " 
                         #_(s/explain-str ::unique-rotors [left-rotor middle-rotor right-rotor]))
                    {})))
